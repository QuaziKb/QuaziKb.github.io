...
function make_init_texture(width, height) {
	...
	for (let i = 0; i < size; i++) {
		...
		if (i % 2 === 0) {
			...
			for (let j = 0; j < 4; j++) {
				rgba[j] += THREE.MathUtils.randFloatSpread(0.01)
			}
			//enable for 2D
			//rgba[2] = 0.0
		}
...
function main() {
	...
	var sqrt_N_particles = Math.floor(Math.pow(2, 8));
	...
	// SDF geometry, this should be automated somehow
	const sdf_geometry = new THREE.SphereGeometry(1.0, 32, 16);
	const sdf_material = new THREE.MeshPhongMaterial({
			'opacity': 0.5,
			'transparent': true,
		});
	const sdf_mesh = new THREE.Mesh(sdf_geometry, sdf_material);
    sdf_mesh.scale.set(0.25,0.25,0.25);
    sdf_mesh.position.x = 0.5;
	scene.add(sdf_mesh);
